...
installed_version=`cat $directory/VERSION.txt`
...
svn info https://github.com/CongducPham/LowCostLoRaGw/trunk/gw_full_latest | grep "Revision:" | cut -d ' ' --field=2 > $directory/git-VERSION-tmp.txt

#in case we don't have internet connectivity, the file size is 0
git_version_size=`stat -c %b $directory/git-VERSION-tmp.txt`
		
if [ "$git_version_size" != "0" ]
then
	mv $directory/git-VERSION-tmp.txt $directory/git-VERSION.txt
	git_version=`cat $directory/git-VERSION.txt`
	...
	then
		...
		then
			...
			$directory/lora_gateway/scripts/update_gw.sh
			sleep 5
		else
	...
	echo "N/A" > $directory/git-VERSION.txt
	git_version=`cat $directory/git-VERSION.txt`
	echo "svn could not get version info from github, mark as $git_version."
	rm -rf $directory/git-VERSION-tmp.txt
fi	
############################################
...
$directory/lora_gateway/scripts/mongo_repair.sh
