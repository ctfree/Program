...
static PT_THREAD(generate_routes(struct httpd_state *s)) {
	...
	for (r = uip_ds6_route_head(); r != NULL; r = uip_ds6_route_next(r)) {
		...
		SEND_STRING(
				&s->sout,
				buf); // TODO: why tunslip6 needs an output here, wpcapslip does not
		blen = 0;
		ipaddr_add(&r->ipaddr);
		ADD("</a>");
#else
...
static void send_packet_event(void) {
	...
	printf("ID: %u, Value: %d,QoS: %d, batt: %u, counter: %u\n", msg.id,
				 msg.value1, msg.value2, msg.battery, msg.counter);
	...
	uip_udp_packet_sendto(server_conn, msgPtr, sizeof(msg), &server_ipaddr,
...
PROCESS_THREAD(border_router_process, ev, data) {
	...
	/* While waiting for the prefix to be sent through the SLIP connection, the
	 * future border router can join an existing DAG as a parent or child, or
	 * acquire a default router that will later take precedence over the SLIP
	 * fallback interface. Prevent that by turning the radio off until we are
	 * initialized as a DAG root.
	 */
	prefix_set = 0;
	NETSTACK_MAC.off(0);
	 ...
	 * Since we are the DAG root, reception delays would constrain mesh
	 * throughbut.
	 */
	NETSTACK_MAC.off(1);
