       ...
       //adeed
            ...
            continue;
                    ...
                    // Sending the data
                     ...
                     // tx_fifo[tx_fifo_first].buffer[0] = DATA;    // commented
                    cc112x_data_mode();
    ...
    for ( ;; )
    {
      // WAIT_EVENT(&sink_thread_pt);
        while (!FIFO_IS_EMPTY(rx_fifo_first, rx_fifo_last)) {
            ...
            uart_putchar(' ');
            memcpy(&pkt, rx_fifo[rx_fifo_first].buffer + 2, sizeof( pkt));
    ...
    for ( ;; )
        {
           // WAIT_EVENT(&source_thread_pt);
            ...
            pwmac_send((uint8*)pkt, SINK_ADDRESS);
