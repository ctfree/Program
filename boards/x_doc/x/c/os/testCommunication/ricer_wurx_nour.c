...
static PT_THREAD (sender_thread(void *args))

  {
    ...
    for ( ;; ){
      ...
      /*  uart_puts("before sleeping\n");


        if ( P2OUT & (1 << 5)){

            uart_puts("on a l'interuption \n");

        }

        else

            uart_puts("on a pas l'interuption \n");
           /* cc112x_continuous_rx();
            WAIT_EVENT(&_recv_thread_pt);*/


       //uart_printf(" sending frame avant : %d\n ",debug);
     //  cc112x_sleep();

        WAIT_EVENT(&_sender_thread_pt);
     // WAIT_EVENT (&_sender_thread_pt,(P2IES &(1 << 5))== 1));



       // debug = P2IFG & (1 << 5);
       // ENABLE_WURX_IT;
     /*   uart_puts("apres sleeping\n");
       ...
       //PT_WAIT_UNTIL(&_sender_thread_pt,   (P2IFG & (1 << 5)) == 1);

     //  uart_printf(" sending frame apres : %d\n ",debug);
      // DISABLE_WURX_IT;
  //    seq = 0u;
