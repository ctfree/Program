...
void waitSearchAck(){
	...
	int packetSize, f = 0, snr = -200, tempn;
	String buf = "cjpA" + sid;
	...
	while((millis()-start) < 2000){ 
		...
		if (packetSize){
			...
			if((buf.equals(wReceive)) && (tempn > snr)){
				snr = tempn;
				gateway = tempg;
			}
...
void loop() {
	if (millis() - lastSendTime > interval) {
		lastSendTime = millis();   
		readDht();
		if((pTemperature != temperature) || (pHumidity != humidity)){
			sendDht();
...
byte onReceive(int packetSize) {
	...
	Serial.println("Snr: " + String(tempSnr));
	Serial.println();
	if(packetType == 'T'){
		sendMessage("ack", sender, 'A', incomingMsgId);
	}
	else if((packetType == 'I') && (sender != gateway) && (tempSnr > currentSnr)){
		gateway = sender;
		currentSnr = tempSnr;
		handOver();
	}
